...
scheduler_t* SchedulerCreate(void)
{
	...
	scheduler->queue = PriorityQCreate(CompareNextRunTime);
...
UID_t SchedulerAddTask(scheduler_t *scheduler, action_func *func, void* param, size_t interval)
{
	...
	status = PriorityQEnqueue(scheduler->queue, (void *)task);
	if (success != status)
	{
		TaskDestroy(task);
		return badUID;
...
int SchedulerRemoveTask(scheduler_t *scheduler, UID_t uid)
{
	...
	task = (task_t *)PriorityQErase(scheduler->queue, CompareTaskUid, &uid);
	if (success != status)
	{
		return fail;
	}
...
void SchedulerClear(scheduler_t *scheduler)
{
	...
	while( 0 != PriorityQSize(scheduler->queue))
	{
		current_task = PriorityQDequeue(scheduler->queue);
...
int SchedulerRun(scheduler_t *scheduler)
{
	...
	{
		current_task = PriorityQPeek(scheduler->queue);
		...
		PriorityQDequeue(scheduler->queue);
		if(0 == status_cercler)
		{
			TaskUpdateNextRunTime(current_task);
			status = PriorityQEnqueue(scheduler->queue, current_task);
			if(success != status)
			{
				TaskDestroy(current_task);
				break;
